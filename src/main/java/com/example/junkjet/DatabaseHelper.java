...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // Method to get the email if it exists
    // Method to check if the email exists in the database
    public boolean doesEmailExist(String email) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT " + COL_2 + " FROM " + TABLE_NAME + " WHERE EMAIL = ?",
                new String[]{email});

        boolean exists = cursor != null && cursor.moveToFirst();
        cursor.close(); // Close the cursor to avoid memory leaks
        return exists; // Return true if email exists, otherwise false
    }
